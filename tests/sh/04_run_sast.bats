...
setup() {
  ...
  export SCRIPT_PATH="${REPO_ROOT}/04_run_sast.sh"
  export TMP_ROOT
  TMP_ROOT="$(mktemp -d)"
...
@test "R005,R010,R015: runs make sast from repo root and prints success" {
  #R005: Repository-root execution traceability.
  #R010: make sast delegation traceability.
  #R015: Success guidance traceability.
  create_make_stub 0
  cd "${TMP_ROOT}/caller"
  run env PATH="${STUB_BIN}:/usr/bin:/bin" MAKE_LOG="${MAKE_LOG}" "${SCRIPT_PATH}"
  [ "$status" -eq 0 ]
  [[ "${output}" == *"SAST checks passed"* ]]
  run rg "^PWD=${REPO_ROOT}$" "${MAKE_LOG}"
  [ "$status" -eq 0 ]
  run rg "^ARGS=sast$" "${MAKE_LOG}"
  [ "$status" -eq 0 ]
}

@test "R020: propagates make sast failures before success output" {
