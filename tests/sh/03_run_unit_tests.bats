...
setup() {
  ...
  export SCRIPT_PATH="${REPO_ROOT}/03_run_unit_tests.sh"
  export TMP_ROOT
  TMP_ROOT="$(mktemp -d)"
...
@test "R005,R010,R015: runs make test from repo root and prints success" {
  #R005: Repository-root execution traceability.
  #R010: make test delegation traceability.
  ...
  [ "$status" -eq 0 ]
  run rg "^ARGS=test$" "${MAKE_LOG}"
  [ "$status" -eq 0 ]
}

@test "R020: propagates make test failures before success output" {
